---
author: Sion
pubDatetime: 2025-05-10T00:09:00Z
modDatetime: 2026-04-21T00:09:00Z
title: Majin Tensei 
slug: majin-tensei
featured: false
draft: true
tags:
  - majin-tensei
description: Majin Tensei notes.
---

## General Information

Hero 100% power:
- Majin, Jashin, Ryuuou, Majuu, Youjuu, Yousei, Maou, Touki, Jaki

Agilao 100% power:
- Ryuuou, Touki, Jaki

Bufula 100% power:
- Majin, Ryuuou, Touki

Zio 100% power:
- Jaryuu, Reichou, Datenshi

Zan 100% power:
- NONE D:

Orthrus (Majuu) 100% power:
- Majuu, Youjuu

Birds (Reichou) 100% power:
- Jaryuu, Reichou, Datenshi - everything else 10%

Always keep an eye for any armor drops and make sure to equip them.

Name the hero.
- 5 STR; 3 VIT

Stats on Hero:
- Defense until 8 (at lvl 4)
- STR until 12 (at lvl 6)
- Defense until 10 (at lvl 8)
- Strength until 28 
- Magic until 7


Stats on Heroine
- Magic until 32 (at lvl25)
- Defense

Keep an emergency dis-paralyze or dis-poison with you.

## Map 1

1: Straight right.

2: Move to fountain.

3: Attack Sylph.

## Map 2

1/2/3: North

4: Attack Kobold to your left

5: Left

6: Attack 

7: Hopefully building

## Map 3 - 1

1: Attack 

2: Right

3: Straight right

4: Down, attack

5: Right

6: North

8: Chest

9: Equip attack knife, attack

Go to stairs.

## Map 4 - 1

1: Up

2: Left

3: Left, summon Orthrus behind you

4: Left, Orthrus follows

5: Orthrus attacks Pixie, Hero advances

6: Hero attacks Laughing Skull, Orthrus advances

Go left, Cerberus collects bottom chest of the two. If you have an extra turn, he can collect the top one.

Kill the Werecat first. Equip the Battle Suit and go up.

## Map 5 - 1

1: Up right, attack Laughing Skull

2: Right, summon Orthrus diagonal down right

3: Orthrus collects left chest, Hero heads right

4: Orthrus returns to Comp. Hero attacks.

5: Kill Goblin.

6: Kill Turdak.

7: Move left.

8: Move left down, summon Orthrus behind you.

9: Hero left, Orthrus down.

10: Orthrus collects bottom left chest, Hero equips the Battle Gloves and attacks.

Once the Goblin is dead, use a Smoke Bomb.

## Map 3 - 2

Re-enter map 3.

Collect the bottom chest with Hero this time. Fight everything. Then smoke bomb.

## Map 3 - 3

Fight everything.

## Map 4 - 3

Rush to stairs through the top.

## Map 5 - 3 

Complete chapter.

Heroine stats:
- Magic to full

## Map 6

**1**: Hero unequips Armor. Heroine equips it. Straight up for both.

**2**: Hero up, Heroine up.

**3**: Hero left, Heroine Agilao Weredog.

**4**: Hero attacks Sylph, Heroine Maragion.

**5**: Hero follows enemies, Heroine Agilao.

When it's only Sylph left for Heroine, summon Orthrus to kill it.

## Map 7

**1**: Both right.

**2**: Both up.

**3**: Hero straight up, Heroine on the road diagonal down from the right building.

**4**: Hero attacks Goblin, Heroine Agilao/Maragion/

**5**: Kill last demons.


## Map 8

Go straight through the middle mc in front. 

Turn 3 - Minotaur should be in reach - SAVE

Hero - Attacks, Heroine Maragion

## Map 9

**1**: Hero move up left, summons Orthrus. Heroine moves left **stays in the forest**.

**2**: Hero attacks left, Heroine Maragion.

**3**: Heroine Agilao. Orthrus moves slightly right.

Hero attacks the Dodongo.

**4**: Hero grabs chest.

Heroine equips the Battle boots. Uses Maragion. 

Orthrus kills the Pooka.

**5**: Hero kills the Sylph. Orthrus goes after the Spartoi.

Finish it next turn.

## Map 10

**1**: SAVE. Hero recruits closer Siren. Heroine moves up, Sabatoma the Siren.

**2**: SAVE. hero recruits other Siren. Heroine moves past bridge, summons Siren.

Orthrus follows past bridge, Siren goes up river.

**3**: Hero recruits Badb Catha if in reach.

Orthrus/Heroine cross bridge. Sirens attack Orias up.

**4**: Sirens take care of last fallen.

Orthrus attacks Tanki, Heroine Maragion the first beast.

**5+**: Clean up.

## Map 11

If you missed Badb Catha, you can get it here.

1: SAVE, recruit Bogle, Heroine north.

2/3: Hero and Heroine north. Maragion when needed.

After Raksha Lango is dead, summon a Siren.

!! Grab left chest of the bottom.

## Map 12

1: Hero grabs chest up, Heroine down Maragion
2: Heroine grabs chest up

## Map 13

1: Maragion two enemies below her, Hero attacks
2: 

## Outside

Go down to the mansion, fuse:
- Bogle x Siren = Dullahan

## Map 14

1: Hero all the way up. Heroine all the way down (grass), summon Dullahan above.

2: Heroine all left, Dullahan follows. Hero all left.

3: Heroine Maragion. Dullahan Zan the sylph. Hero left to attack.

Finish with AOEs.

## Map 15

1: Up both

2: Up both

3: All the way right both, stay on grass

4: Hero attack Puuka, Heroine all the way right

5: Heroine Maragion both Majuus. Hero gets on bridge, summon Orthrus on bridge and Siren diagonal top right.

6: Siren goes far into mountains. Orthrus crosses bridge far into plains. Heroine uses Maragion. Hero defeats another Majuu.

7: Hero goes on bridge. Siren goes north. Orthrus goes through bridge. Heroine uses Magic no whoever.

8: Hero attacks the Toukis. Cerberus goes further north through mountains. Siren gets on Generator. Heroine magic on remaining Youjuu.

9: Hero goes North. Cerberus grabs left chest with Aquamarine.

10: Save. Hero recruits Elf (if not in reach, Sylph). Cerberus attacks Hanbaba.

## Map 16

1: Hero goes right, summon Orthrus, Dullahan, Siren, Elf behind. Heroine north:

2: Heroine north. Hero attacks leftmost enemy. Orthrus goes to generator, everyone advances. Heroine north.

3: Hero attacks leftmost Touki, Siren attacks Yurlungur. Dullahan Mazanma remaining enemies, Cerberus attacks as well. Heroine north.

4: Heroine north, should be able to Bufula the Ananta. Dullahan Mazanma the Toukis, Siren goes to Generator. Hero goes straight north.

5: Siren attacks the bird. Elf heals Dullahan moves to generator. Hero moves right, Heroine bufula Ananta.

6: Save. Hero attacks Moloch. Heroine Maragion.

## Map 17

1: Hero next to house. Siren diagonal up, Orthrus diagonal down, Heroine goes to diagonal up.

2: Save. Orthrus attacks Sylph. Heroine uses Maragion. Hero moves to fountain attacks, Tetzaquotl (can miss). Siren goes left up.

3: Orthrus down attacks Turdak. Hero attacks Elf. Heroine gets on Fountain, Maragion. Siren attacks Spartoi.

4: Orthrus heads further down to bait Spartoi. Heroine Maragion. Hero kills last Baykok. Siren goes left to bait the Loa.

5: Orthrus goes north. Hero and Heroine left, Heroine Dirahan herself. Siren tries to bait Loa

6: Heroine uses Maragion. Hero attacks Loa. Orthrus goes left, Siren tries to bait Loa.

Close it out, hopefully nobody dies to Poison.

## Outside

Go to the mansion:
- Dullahan x Purski = Rusalka
- Elf x Rusalka = Ganesha

## Map 20

1: Right both

2: Right both

3: Heroine up, Hero 

4: Right both, Heroine up sabatoma Siren up

5: Heroine Zionga, Siren attacks Yurlungur. Hero right.

6: SAVE. Hero recruits Jatayu. Heroine moves up, Siren attacks Yurlungur in fountain, if its dead already move to Hero.

7: Hero attacks Typhon, hopefully kills. Heroine goes to fountain, summons Ganesha behind. Put Siren in safety.

8: Heroine Maragion. Hero summons Jatayu. Ganesha moves to Fountain.

9: Mabufula, Maragion til the end.

## Map 22

1: Hero left, summon Jatayu and Ganesha. Heroine left.

2: Hero and Heroine up, Siren up, Ganesha against the wall.

3: Ganesha attacks with Bufula the Baykok. Heroine uses Zionga on Apis, Hero up. Siren up.

4: Ganesha uses Bufula. Heroine uses Zionga on Yatagarasu, Hero attacks. Siren up.

5: Hero continues. Heroine advances, sabatoma Uriel. Siren up. Ganesha Bufula.

6: Ganesha into the corridor, Hero continues, Uriel behind him then Heroine. Siren picks up the Helm.

7: Hero equips Helm continues along the wall. Uriel moves close to generator. Ganesha far into corridor. Heroine follows.

Hopefully you can kill Saleos with Maragion and Hero takes the stairs. 

## Map 23

1: Hero straight up summon Jatayu up, Ganesha and Uriel down. Heroine up.

2: Jatayu up left. Move up in this order against the wall: Hero, Ganesha, Heroine. Uriel under.

3: Ganesha uses Mabufulu (bufula doesnt kill (!)). Heroine uses Maragion. Jatayu, Uriel and Hero move left.

4: Hero attacks the Isora. Heroine uses Zionga the Yurlungur. Everyone moves left down.

5: Moving left.

6: Moving left and down.

7: Heroine/Jatayu and Ganesha will stick around the generator, Uriel and Hero advance down.

8: Uriel attacks the Raksha Lango to protect Ganesha from all the spawns, everyone moves down.

9: Save. Hero down, kill enemies with Maragion/Mabufula, watch out for Ganesha's health.

10: Siren to generator, Uriel and Hero down.

Mabufula the Basilisk through the wall, move Hero up the corridor. If you get paralyzed, your only hope is heroine helping.

## Map 24

1: Heroine uses Maragion on Basilisk. Hero moves to generator.

2: Heroine uses Agilao on Siren. Hero takes one step and summons Siren on the generator.

3,4,5,6,7,8: Move through the left

Once uriel is in reach, attacks Yatagarasu.

9?: When right in front of the enemies basically with Heroine and Hero, summon Ganesha and Jatayu.

10: Heroine -> Zionga on Yurlungur. Hero attacks Dryad if she's there, otherwise walks to generator.

11: Hero attacks Orthrus. Heroine -> Maragion Medusa. Move Uriel and Jatayu next to Heroine. Ganesha Mabufula.

12: Save. Ganesha -> Mabufula Medusa. Heroine -> Maragion Medusa. Hero attacks.

## Map 25

1: Save. Hero recruits Dryad. Heroine -> Maragion the Basilisk.

2: Save. Hero recruits Saleos. Heroine -> Sabatoma Ganesha.

3: Save. Hero recruits Houou. Heroine -> Maragion Bugaboo.

4: Save. Hero recruits Typhon. Heroine -> Sabatoma Typhon in front of Cerberus.

5: Save. Hero recruits Loki. Typhon attacks Cerberus. Heroine walks to stairs.

## Map 26

1: Hero goes left, summons Uriel and Jatayu, Heroine goes right.

2: Hero/Heroine up, Uriel up for right top chest, Jatayu goes for top of the bottom right chests.

3: Hero/Heroine up, demons move.

4: Uriel collects Beast Hunter - don't equip. Jatayu picks up Kurenai-no-Kabuto, Hero equips.

5: Jatayu collects other chest there, can replace a Magic Bottle. Heroine Maragion the guard.

## Map 27

1: Hero and Heroine move bottom left.

2: Hero goes as far as he can, Heroine tile right under.

3: Hero moves to attack Tamuzu, Heroine summon Typhon, Uriel and Jatayu around her.

4: Jatayu goes down, Uriel straight up. Typhon attacks the Baykok, move to the corridor.

5: Jatayu grabs the rightmost chest, Kurenai-no-Kote. Hero equips it. Uriel up. Hero attacks the Baykok, put someone in the generator.

6: SAVE. Hero recruits Houou. Jatayu grabs the other chest (Dis-paralyze). Uriel grabs the right chest (Pearl). Heroine leaves.

Leave the tower.

## Map 28

1: Hero recruits Houou. Heroines moves one tile up, sabatoma (left to right) -> Houoh, Houoh, Houoh, Jatayu, Uriel,Jatayu, Siren.

2: Hero recruits Makara. Birds go straight left forming a wall, Uriel goes above to the mountains.

3: Siren kills Yatagarasu in front of Uriel. Uriel goes straight forward. Birds fight the birds.

4: Uriel attacks the Apis/Maya. Houou/Jatayu finish killing the Maya, others advance. Hero/Heroine advance.

5: Uriel moves to generator, birds advance but stay on water. Hero advances. Heroines moves 1 tile away from fire corridor, sabatoma -> Cerberus in front, Ganesha down left, Mahakara down.

6: SAVE. Move a bird to the other generator, advance the others, try to grab both chests if you can already, if so Hero equips. Attack with Uriel. Cerberus attacks, Ganesha Mabufula, Heroine Maragion. Hero advances.

7: Uriel and birds attack Loa, if Loa is <10HP the birds kill. Get top chest. Heroine Maragion. Cerberus attacks the empusa, Ganesha Bufula to kill it. Hero equips the Kurenai-no-Gusoku.

8: Uriel attacks Loas. Ganesha Mabufula the empusa. Heroine Maragion the Bugaboo. Hero attacks the Boogle.

9: Uriel cleans up (cover generator just in case). Heroine uses Maragion on Bugaboo. Ganesha uses Mabufula on Goblin. Mahakara uses Mazan on the Bogle, Hero kills it.

---

After the stage, go left in the map for the next stage.

## Map 29

1: Hero straight up -> summon Mahakara behind, Loki front and Jatayu bottom left, Heroine right.

2: Hero straight up, summons Ganesha behind, Houou on each side of Ganesha. Heroine straight up, sabatoma -> Cerberus up, Uriel right. Jatayu straight left, Loki and Mahakara down.

3: Ganesha uses Mabufula. Both Houou attacks Mayas. 

Cerberus attacks Loki, Heroine Bufula to kill it. Uriel attacks the Shitori if its close to die or Yatagarasu. 

Hero kills the remaining Maya. Jatayu straight left. Mahakara up the third corridor.

4: If Papilsag are in reach, attack with Maragion. Mahakara go up. Jatayu go up.

Kill the Yatagarasu, Jatayu and Papilsag. If you can kill Papilsag without Cerberus, advance him up, otherwise keep him around there. Move Hero up.

5: Ganesha Mabufula the Basilisk, Hero attacks the Sylph. Mahakara attacks the Baykok. Jatayu goes up. Move a bird to the generator and Uriel the chest top right.

6: Mahakara kills the Bugaboo. Move Uriel and Jatayu up. Move Hero and Ganesha to the last Basilisk.

7: Uriel grabs the Sun Mask. Heroine equips the Sun Mask. Jatayu grabs the Armor. Hero equips the Armor and attacks the Basilisk.

---

Go back all the way to the right for the next stage.

## Map 31

1: Hero up around the house, summon Cerberus right, Thor left. Heroine right.

2: Save. Hero attacks Baykok, Cerberus attacks the top Loki. Thor moves straight forward Heroine moves right, summon Mahakara, Loki, Houou, Uriel behind.

3: Houou attacks the Jatayu. Thor uses Mazio to Apis and Yatagarasu. Heroine uses Zionga on Yatagarasu.

Hero attacks the Papilsag. Mahakara and Loki attacks the Loki. Uriel heals the Cerberus.

4: Uriel attacks Shitori. Thor Mazio hopefully all 3. Loki Zanma the Orobas, Heroine Zionga the Orobas to hopefully kill.

Mahakara uses Mazan on Shitori and Maya, Houou hopefully kills Maya. Make sure these all die, otherwise they'll go to the fountain or use Diarahan.

If everyone is taken care of, move Cerberus and Hero to the remaining dragons.

5: SAVE. Cerberus attacks the Maya down, hopefully kill. Loki Zanma a Maya, move Heroine (Attack if needed). Hero attacks.


## Map 32

1: Hero left. Heroine go up, summon Uriel right under the fire.

2: Uriel up, Heroine left. Hero left, summon Ganesha right behind him, Cerberus and Mahakara up and down Ganesha.

3: Ganesha uses Mabufula. Everyone moves further left, Uriel goes to Generator.

4: Everyone goes left, Heroine summon 3 Houou in front.

5: Mahakara uses Mazanma on Bugaboo, Hero and Cerberus advance, attack the Bugaboos. Birds and Heroine kill the Apises.

6: Hero/Cerberus/Mahakara go left, Mahakara can recover in fountain if needed. Send all birds to the middle generator and Ganesha and Heroine to the left.

7: Birds to generator, move the Houhous in the direction of the Hero. Everyone through the path.

8: Everyone through the path. Houhou close to Hero. Hero summons Undyne

9: Hero moves up 1 tile away from the exit of the fire corridor (protect from multiple magic attacks). Cerberus stands 1 tile away. Houhou moves close. Heroine/Mahakara move close.

10: Cerberus attacks the Orcus, Hero attack the Orobas.  ?Houhou attacks the Orobas?. Heroine and Undyne up.

11: Cerberus attacks Empusa, Mahakara attacks Orcus. Undyne heals Hero. Hero attacks Orcus. Heroine up.

12: All move right, move Houhous right too.

13/14?: Move Hero as far as the first tile next to fire and water. Move all the demons and Heroine to the start of the fire corridor.

15: Send Cerberus down the corridor, put everyone behind him not in magic range. Houhous go fight the forces.

16: Try to take fountain with Houhou/defeat a Maya. Attack Belial with Cerberus. Try to put another demon in magic range of Belial to bait him. Move Hero and Heroine closer.

17: Hero kills.

## Map 33

It's okay if some birds die in this stage. You need at least 3 birds and Uriel alive by the end.

1: Hero moves up, summons Uriel left, Heroine up Sabatoma -> Houhou and Jatayu each side.

2: Hero/ Heroine/ Cerberus as far up as possible, Uriel up, Birds up in zigzag.

3: Kill Maya and Orobas with birds, move them up in a line. Everyone else up.

4: Uriel straight right, Hero/Heroine up. Keep all birds in the void in zigzag.

5:

## Map 35

1: Hero and Heroine go diagonal up, summon 2x birds on one side and 1x bird and Uriel on each side.

2: Go straight up.

3: Go straight up, not in range for magic.

4: Straight up, line up with chests.

5: Grab all 4 chests. Hero steps down and exits stage.


Go to the Rag's store and exchange the pearls for the Ora Armor and Ora Helm.

Enter stage 35 again to equip the armor and exit.

Go back and grab Raphael

## Map 34

Equip the Ora Armor and Ora Helm on Hero. Equip the remaining good armor on the Heroine.

1: Hero summons a Jatayu on top of the island and Heroine summons Raphael and a Houou.

2: Jatayu grabs the sword, Raphael and Houou move through the gap.

3: Jatayu returns to COMP, Raphael/Houou move through gap.

4: Raphael/Houou cross the continent to the right side of the gap.

5: Move up.

6: Move up, don't attack yet.

7+: Houou attacks the rightmost Fafnir, Raphael heals.

Once the Fafnir dies, Raphael picks up the chest. Then Traesto out.

## Map 36/37/38

Walk to the corner of the room.

## Map 39

1: Hero goes left, summons Raphael, Cerberus and 2x Houou. Hero goes up and summons Uriel and other Houou/Jatayu.

2: Raphael goes left, Houou follow and kill the Kobalt. Uriel goes up. Take out the Gargoyles. Hero follows bridge up

3+: Same movements. Hero follows the path.

There's no strategy to these stages.

6: Uriel and Raphael cover the generator. Houous go up.

At some point replace Jatayu with Uriel in the generator so he can move and clean up any leftover demons.

## Map 41

1: Hero up, summons Cerberus front, Raphael left of Cerberus, Houou right. Heroine up, Zionga the garuda.

2: Cerberus attacks Behemoth, Heroine Bufula it. Houou attacks the Apis. Raphael attacks the injured Garuda. Hero moves up, summons left Uriel, Gabriel and Ganesha behind.

3: Ganesha uses Mabufula on two Behemoths. 

## Map 42

Review addition: get Hydra

It's okay if Orthrus/Typhon/Thor/1 Houou die. The behavior of the demons in this stage is really unpredictable so each stage has a suggestion of which demons really need to die. Evaluate as you go.

1: Hero straight right (approaching Baal and Phoenix to recruit) summons Houou and Jatayu. 

Heroine moves 1 tile up, summons (top to bottom, left to right) Raphael, Typhon Houou, Thor, Orthrus, -, Cerberus, Mahakara.

2: Houou and Jatayu kill first dragon. Hero recruits Baal/Phoenix if in range.

Heroine Maragion if she can hit multiple.

Otherwise, Orthrus -> Stonka. Heroine -> Bufula the Touki. Cerberus attacks Tetza. Move Houou to the left of the Heroine.


3: Hero (hopefully) recruits Baal. Houou/Jatayu kill the Fafnir. 

Houou attacks the Maya. Heroine -> Zionga Gamijin. Raphael -> attack and kill Gamijin. 

Typhon attacks Typhon. Cerberus attacks Rangda.


4: Heroine Mabufula to kill Hydra. 

5: Cerberus attacks Fafnir. Orthrus attacks Isora.

Heroine summons Gabriel, Phoenix and Baal.

6: Kill Orkus with Mabufula.

7: Try to Mabufula Gozuki and one of the other ones. Gozuki should die to one Mabufula.

Baal uses Zionga on Phoenix or Garuda. Others attack as you see fit.

8/9: Clean up last ones with Mabufula and attacking.

## Map 43

It's okay if Mahakara dies.

1: Hero up, Heroine around ice down.

2: Hero up -> summon Cerberus, Gabriel and Raphael above, Phoenix right. Heroine down.

3: Heroine to fountain, Uriel up, Mahakara down, Baal left. 

Gabriel goes to fountain, Phoenix diagonal down left of him, Cerberus diagonal up right. Raphael diagonal down right of Cerberus.

Hero goes up left.

4: Heroine Mabufula, Uriel heal Baal.

Hero and Baal attack bottom Mahakara, Mahakara attacks the top one.

Raphael/Gabriel/Phoenix try to kill both Leonards.

Cerberus attacks one of the surrounding enemies.

5: Heroine Mabufula the other Hydra.

Hero and Baal attack Mahakara, Uriel heals Baal.

Gabriel/Raphael heal if needed. Cerberus attacks one of the enemies.

6: Heroine goes over to the left side, Maragion an enemy if in reach.

Baal and Hero kill the remaining Mahakara. Uriel heals if there was a miss, otherwise attack a Dryad with Phoenix.

Gabriel/Raphael kill the remaining Dryads.

7: Cerberus attacks one of the Zomas. 

Heroine Mabufula zomas, Hero and Baal approach.

8: Mabufula clean up.

## Map 44

1: Hero straight down, summon below all angels and a Phoenix, 2 Houous and Cerberus. Heroine attacks the close enemy.

2: Uriel goes to generator 1.

Michael, Hero, Heroine and Cerberus go left.

Everyone else goes down.

3: Houous attack Leviathans. One angel covers generator 2.

Michael and crew go left, other one goes down.

4: One bird follows an angel down to generator 3.

Michael approaches generator 1.

Other bird follows Michael. Rest is fighting whatever is in range.

5: Bird and angel attack Leviathan from generator 4.

Michael goes down to generator 3. Another angel covers generator 1. Everyone advances down.

6: Make sure all generators are covered. Slowly fight the leviathans.

If you still have one, summon a Jatayu.

Hero and Heroine come down, hopefully bait the Leviathans.

7+: Heal heroine if needed with Angel. Heroine Zionga a Leviathan if in range. Chip away.

## Map 46

Godslayer on Hero makes him do more damage against the Zomas

Summon Michael, Gabriel, Baal, Cerberus. Heroine summons Phoenix on the first generator turn 1.

The rest will come later...


## Map 47

...

## Map 48

1: Hero walks up. Heroine goes down.

2: Hero walks up.

3: Hero walks up.

4: Hero talks to Isis. Heroine uses a smoke bomb or traesto.

## Map 40

........

## Map 49

**1**: Hero goes right, summon Raphael, Gabriel and Michael. Heroine activates the glitch.

**2/3/4/5**: SAVE. Angels go right. activate menu.

**6**: SAVE. All 3 rampage the Oses on top of Astoroth. Open menu at the end. Keep doing this til Astoroth dies.

## Map 50

**1**: Hero moves up next to fountain, summons Baal, both Tiamat and Yamata no Orochi, Michael. Heroine moves up right, opens menu.

**2**: SAVE. Heroines moves up right further. Baal and Tiamat attack the left demons.

Other Tiamat moves right, Michael moves right up. Open menu.

**3**: SAVE. Right and left Tiamat Rampage.

Baal and Yamata move further left, Michael further right.

Open menu. Heroine moves up.

**4**: SAVE. Tiamat kills last demons. Michael moves.

Heroine opens menu moves up. Left demons also move up.

**5**: Heroine Mabufula. Michael Rampage. 

Open menu and move the other ones.

**6+**: Rampage 

## Map 51

**1**: Open menu. Hero up.

**2**: Hero attacks Behemoth. Heroine opens menu.

**3**: SAVE. Move to the shrine, open menu.

**4**: SAVE. Walk to shrine.

## Map 52: 

**1** Heroine open menu walk up to the right.

**2/3/4/5**: Straight up.

**6**: Save. Walk back in bounds.

## Map 53

**1/2**: SAVE. Heroine walks left, opens menu.

**3/4/5/6/7**: Heroine walks up.

**8**: Heroine walks up, one tile away from getting the attention of demons.

**9**: Heroine sabatoma a Tiamat next to the others. Hero opens menu.

**10**: Tiamat Rampage, Heroine Mahabufula.

**11**: Heroine walks to stairs.

## Map 54

**1**: Open menu. Heroine up.

**2** Save. Heroine up to the corner. Hero opens menu.

**3**: Heroine up.

**4**: Heroine up, sabatoma Tiamat under.

**5**: Mabufula Rampage

**6/7**: Mabufula Rampage and then walk to stairs.

## Map 55

**1**: Hero and Heroine move up to the middle of the demons without range.

**2/3**: Up

**4/5**: Up only until you're out of range for Baal's Zionga

**6/7**: Up

**8**: Hero up 2 tiles away from enemies, Heroine all the way up.

**9**: SAVE. Hero summons to his left both Tiamat, Michael and Gabriel, Phoenix.

**10**: SAVE. Michael goes up to the middle, Rampage.

Tiamat and others follow, Rampage.

Heroine Zionga the Leonard. Gabriel Rampage the Gamijin.

Hero opens menu, goes up.

**11**: Heroine Mabufula. Michael Rampage.

Tiamat's Rampage with Lucifer in range. Gabriel Rampage the Gamijin.

HERO OPENS THE MENU.

**12**: Kill

## Map 56

**1/2/3**: Open menu, both walk up.

**4**: SAVE. Hero walks through the grassery, summons top to bottom, Gabriel, Raphael, Michael.

Heroine opens menu, walks up, stays on grass.

**5/6**: Save. Everyone to the right, open menu at the end!

**7**: Raphael and Gabriel Rampage the Leonards/Dagon.

Michael Rampage Vitra.

The crew advances, open menu!!

**8+**: Angels use Rampage, Hero and Heroine approach, open menu always at the end.

## Map 57

**1:** Open menu, walk up.

**2**: Walk up, avoid the demon range.

**3**: Walk to teleporter.

## Map 58

**1**: Open menu, walk up 2 tiles.

**2**: SAVE. Walk around and down, open menu.

**3**: SAVE. Walk to teleporter.

## Map 59:

**1**: Open menu. Heroine clips onto wall.

**2**: SAVE. Heroine gets off wall, goes to middle, Sabatoma -> 2x Tiamats, Michael, Shiva.

**3**: SAVE. Michael Rampage both Fallen Angels in the front, Shiva/Heroine help make sure they're dead.

Tiamats Rampage Satan.

GGs



